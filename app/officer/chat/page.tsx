...
const QUICK_SUGGESTIONS = [
    { text: 'How do I assign a technician?' },
...
export default function OfficerChatPage() {
    ...
    const [messages, setMessages] = useState<ChatMessage[]>([
        {
            sender: 'bot',
            text: `Hello Officer! Welcome to FixCity Chat. I can help you with complaint verification, technician assignment, priority management, and analytics. What do you need?`,
            timestamp: new Date(),
        },
    ]);
    ...
    const handleSubmit = async (messageText?: string) => {
        ...
        try {
            const response = await fetch('/api/chat', {
                ...
                body: JSON.stringify({ role: 'Officer', message: textToSend }),
            });
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p className="text-xs text-gray-400">Officer Assistant</p>
                            </div>
                        </div>
                    </div>
                    <div className="px-3 py-1.5 bg-blue-500/20 border border-blue-500/30 rounded-lg text-xs text-blue-400">
                        👮 Officer
                    </div>
                </div>
            </header>
            ...
            <div className="max-w-4xl mx-auto px-4 py-6 flex flex-col" style={{ height: 'calc(100vh - 80px)' }}>
                ...
                <div className="flex-1 overflow-y-auto mb-4 space-y-4">
                    {messages.map((msg, index) => (
                        ...
                        >
                            <div
                                className={`w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 ${msg.sender === 'bot' ? 'bg-blue-500' : 'bg-white/10'
                                    }`}
                            >
                                {msg.sender === 'bot' ? (
                ...
                <div className="bg-white/5 border border-white/10 rounded-xl p-2 flex items-end gap-2">
                    <textarea
                        ...
                        placeholder="Ask about managing complaints, assignments..."
                    ...
                    <button
                        ...
                        className="bg-blue-500 hover:bg-blue-600 text-white p-3 rounded-lg transition-all disabled:opacity-50 disabled:cursor-not-allowed flex-shrink-0"
                ...
                <p className="text-center text-xs text-gray-500 mt-2">
                    Verify important information with records if needed.
                </p>
            </div>
        </div>
    );
